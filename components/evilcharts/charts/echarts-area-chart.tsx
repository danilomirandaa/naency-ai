      ...
      .map((param) => {
        ...
        const dimmed =
          ...
          (hovered != null && hovered !== key)
            ? " opacity-30"
            : "";
        // Naency: o tooltip é HTML, então formatValue devolve string (no twin do
        // Recharts é ReactNode). Sem ele, cai no número puro em pt-BR.
        const value =
          typeof p.value === "number"
            ? (item?.formatValue?.(p.value) ?? p.value.toLocaleString("pt-BR"))
            : String(p.value ?? "");
